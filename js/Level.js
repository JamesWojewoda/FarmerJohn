...
(function (window) {
    ...
    function Level(stage, contentManager, textLevel, gameWidth, gameHeight) {
        ...
        this.textTiles = Array.matrix(15, 155, "|");
        // Physical structure of the level.
        this.tiles = Array.matrix(15, 155, "|");
    ...
    Level.prototype.ParseLevelLines = function (levelLine) {
        for (var i = 0; i < 15; i++) {
            for (var j = 0; j < 155; j++) {
                this.textTiles[i][j] = levelLine.charAt((i * 154) + j);
            }
        }
    };
    ...
    Level.prototype.LoadTile = function (tileType, x, y) {
        switch (tileType) {
            ...
            case '(':
                return this.LoadNamedTile("ground5", Enum.TileCollision.Impassable, x, y);
                break;

            case ')':
                return this.LoadNamedTile("ground6", Enum.TileCollision.Impassable, x, y);
                break;

            // Impassable block                                                                                      
            case '#':
                return this.LoadVarietyTile("ground", 5, Enum.TileCollision.Impassable, x, y);
                break;
    ...
    Level.prototype.LoadNamedTile = function (name, collision, x, y) {
        switch (name) {
            ...
            case "ground0":
                return new Tile(this.levelContentManager.ground0, collision, x, y);
                break;

            case "ground1":
                return new Tile(this.levelContentManager.ground1, collision, x, y);
                break;

            case "ground2":
                return new Tile(this.levelContentManager.ground2, collision, x, y);
                break;

            case "ground3":
                return new Tile(this.levelContentManager.ground3, collision, x, y);
                break;

            case "ground4":
                return new Tile(this.levelContentManager.ground4, collision, x, y);
                break;

            case "ground5":
                return new Tile(this.levelContentManager.ground5, collision, x, y);
                break;

            case "ground6":
                return new Tile(this.levelContentManager.ground6, collision, x, y);
                break;
    ...
    Level.prototype.LoadEnemyTile = function (x, y, name) {
        ...
        switch (name) {
            ...
            case "MonsterD":
                ...
                break;
            case "Bat":
                this.Bats.push(new Bat(this, position, this.levelContentManager.bat));
                break;
        }
    ...
    Level.prototype.CreateAndAddRandomBackground = function () {
        ...
        backgroundSeqTile1 = new Bitmap(this.levelContentManager.imgBackgroundLayers[0][0]);
        backgroundSeqTile2 = new Bitmap(this.levelContentManager.imgBackgroundLayers[0][1]);
        backgroundSeqTile3 = new Bitmap(this.levelContentManager.imgBackgroundLayers[2][randomnumber]);
        backgroundSeqTile2.x = 3200;
    ...
    Level.prototype.StartLevel = function () {
        ...
        this.levelStage.setTransform(moveBy * -1, 0);
        moveBy = 0;
    ...
    Level.prototype.Update = function () {
        ...
        this.levelStage.setTransform(moveBy, 0);
        this.Hero.tick();
        if (!this.Hero.IsAlive || this.TimeRemaining === 0) {
            ...
            if (!this.IsHeroDied)
                this.UpdateGems();
            if (this.Hero.BoundingRectangle().Top() >= this.Height() * StaticTile.Height||this.Hero.position.x < moveBy) {
                this.OnPlayerKilled();
            }
